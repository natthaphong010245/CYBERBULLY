{{-- resources/views/assessment/cyberbulling/person_action/form/form.blade.php --}}
@extends('layouts.assessment.cyberbulling.person_action.form.form')
@section('content')
    <div class="bg-white w-full flex-grow rounded-t-[50px] px-6 pt-8 flex flex-col mt-8 pb-20">
        ...
        <form id="questionnaire-form" method="POST" action="{{ url('/assessment/cyberbulling/person_action/form') }}">
            @csrf
            ...
            @foreach($questions as $index => $question)
                ...
                <div class="mb-4 question-container" id="question-container-{{ $questionNumber }}">
                    <p class="text-gray-400 mb-4 question-title" id="question-title-{{ $questionNumber }}">{{ $question }}</p>
                    <div class="flex justify-between">
                        @foreach($options as $option)
                            <div class="flex flex-col items-center w-16 option-container" data-question="{{ $questionNumber }}" data-value="{{ $option['value'] }}">
                                <div class="flex items-center justify-center h-8">
                                    <input type="radio" name="question{{ $questionNumber }}" value="{{ $option['value'] }}" 
                                           class="{{ $option['size'] }} border-2 radio-option" data-question="{{ $questionNumber }}">
                                </div>
            ...
            <div class="flex justify-center mt-10 mb-5">
                <button type="submit" class="submit-btn text-lg px-16 py-3 rounded-xl text-white font-medium shadow-md bg-[#929AFF]">
                    ส่งคำตอบ
                </button>
            </div>            
        </form>
    </div>
    @include('layouts.assessment.cyberbulling.percent')
@endsection
